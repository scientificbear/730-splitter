...
if (!existsSync(inputPath) || !existsSync(expectedDir)) {
  console.log(
    'Skipping local regression: test_data/ not found (expected on developer machine only).',
  );
  process.exit(0);
}
...
for (const filename of expectedFiles) {
  ...
  const actualMatch = result
    .get(filename)
    .match(/<m730:TotaleDocumenti>(\d+)<\/m730:TotaleDocumenti>/);
  ...
  if (expectedTot !== actualTot || String(docCount) !== actualTot) {
    console.error(
      `${filename}: TotaleDocumenti=${actualTot} (expected ${expectedTot}), docs=${docCount}`,
    );
    failed++;
  } else {
